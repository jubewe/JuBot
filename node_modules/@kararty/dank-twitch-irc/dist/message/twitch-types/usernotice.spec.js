...
describe("./message/twitch-types/usernotice", function () {
    describe("#extractEventParams()", function () {
        it("should camelCase all properties that start with msg-param-", function () {
            chai_1.assert.deepStrictEqual((0, usernotice_1.extractEventParams)({
    ...
    describe("UsernoticeMessage", function () {
        it("should be able to parse a USERNOTICE with no message, only system-msg", function () {
            ...
            const msg = (0, twitch_message_1.parseTwitchMessage)(msgText);
            ...
            // typescript test:
            if (msg.isResub()) {
                (0, tsd_1.expectType)(msg);
                (0, tsd_1.expectType)(msg.eventParams);
                (0, tsd_1.expectType)(msg.eventParams.cumulativeMonths);
                (0, tsd_1.expectType)(msg.eventParams.cumulativeMonthsRaw);
            }
        });
        it("should be able to parse a resub with message", function () {
            const msg = (0, twitch_message_1.parseTwitchMessage)("@badge-info=subscriber/15;badges=subscriber/12;color=#00CCBE" +
        ...
        it("should be able to parse a masssubgift with message", function () {
            const msg = (0, twitch_message_1.parseTwitchMessage)("@badge-info=subscriber/12;badges=subscriber/12,premium/1;color=;display-name=realuser;emotes=;flags=;id=99b77ba7-c77f-4d92-ac3a-ad556e921672;login=realuser;mod=0;msg-id=submysterygift;msg-param-mass-gift-count=1;msg-param-origin-id=4e\\sd1\\s19\\sc5\\s33\\s80\\s68\\s8c\\sdc\\sc9\\s4d\\s96\\s73\\sd0\\sad\\s40\\s52\\sf3\\s19\\s02;msg-param-sender-count=1;msg-param-sub-plan=1000;room-id=38244999;subscriber=1;system-msg=realuser\\sis\\sgifting\\s1\\sTier\\s1\\sSubs\\sto\\sbroadcaster's\\scommunity!\\sThey've\\sgifted\\sa\\stotal\\sof\\s1\\sin\\sthe\\schannel!;tmi-sent-ts=1633549401426;user-id=239909999;user-type= :tmi.twitch.tv USERNOTICE #broadcaster");
            chai_1.assert.strictEqual(msg.ircCommand, "USERNOTICE");
            chai_1.assert.strictEqual(msg.ircParameters[0], "#broadcaster");
            chai_1.assert.strictEqual(msg.ircTags["msg-param-mass-gift-count"], "1");
            chai_1.assert.strictEqual(msg.systemMessage, "realuser is gifting 1 Tier 1 Subs to broadcaster's community! They've gifted a total of 1 in the channel!");
            chai_1.assert.strictEqual(msg.messageTypeID, "submysterygift");
            chai_1.assert.isTrue(msg.isMassSubgift());
            chai_1.assert.deepStrictEqual(msg.eventParams, {
                massGiftCount: 1,
                massGiftCountRaw: "1",
        ...
        it("should be able to parse an announcement usernotice that was sent via IRC", function () {
            const msg = (0, twitch_message_1.parseTwitchMessage)("@badge-info=;badges=broadcaster/1,glitchcon2020/1;color=#666666;display-name=NotKarar;emotes=;flags=;id=fb6f330a-b47e-4394-bdae-34c545143a1e;login=notkarar;mod=0;" +
            ...
            chai_1.assert.strictEqual(msg.ircCommand, "USERNOTICE");
            chai_1.assert.strictEqual(msg.ircTags["msg-param-color"], undefined);
            chai_1.assert.strictEqual(msg.eventParams.color, undefined);
            chai_1.assert.strictEqual(msg.messageTypeID, "announcement");
        });
        it("should be able to parse an announcement usernotice that was sent via web chat", function () {
            const msg = (0, twitch_message_1.parseTwitchMessage)("@badge-info=;badges=broadcaster/1,glitchcon2020/1;color=#666666;display-name=NotKarar;emotes=;flags=;id=e409ccaf-c439-4438-9153-77a056eab544;login=notkarar;mod=0;" +
            ...
            chai_1.assert.strictEqual(msg.ircCommand, "USERNOTICE");
            chai_1.assert.strictEqual(msg.ircTags["msg-param-color"], "PRIMARY");
            chai_1.assert.strictEqual(msg.messageTypeID, "announcement");
            chai_1.assert.strictEqual(msg.eventParams.color, "PRIMARY");
        });
    });
});
