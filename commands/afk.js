...
module.exports = {
    ...
    exec: async () => {
        ...
        if(["afk", "gn"].includes(j.message._.command)){
            ...
            .then(a => {
                j.send(0, null, `${j.message._.usertag} is now ${getafkopt(a.opt)}: ${afkmessage}`);
            })
            .catch(e => {
                console.error(e);
                j.send(2, null, `Error: Could not set AFK Status Sadge`);
            });
        } else if(["cafk", "rafk"].includes(j.message._.command)){
            _afk(3, j.message.userstate.id, null, null, true)
            .then(a => {
                if([0, 1].includes(a.type)){
                    j.send(0, null, `${j.message._.usertag} Your old AFK status has been resumed: ${a.message}`);
                } else if([2].includes(a.type)){
                    j.send(0, null, `${j.message._.usertag} Your old AFK status has been reused: ${a.message}`);
                }
            })
        } 
