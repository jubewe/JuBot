    ...
    exec: async () => {
        ...
        request(`${urls.api.__url("serverinfo", "GET")}`, {method: "GET", headers: j.vars().j_api_headeradmin()}, (e, r) => {
            if(e){
                ...
                j.send(2, null, `Serverinfo: Uptime: ${_cleantime(dat.pi.uptime, 4, 2).time.join(" and ")}; Memory Usage: ${(_percentage(dat.pi.memory.total, dat.pi.memory.free, null, 0))}% (${Math.round(dat.pi.memory.used/1048576)} / ${Math.round(dat.pi.memory.total/1048576)} mb); \n` +
