...
function _cleannumber(num, numopt, numdigits, numspacer, insertnumname, firstcap){
    ...
    return `${(numspacer ? _numberspacer(num_.substring(0, num_.length-parseInt(numnamenum))) : num_.substring(0, num_.length-parseInt(numnamenum)))}${insertnumname}${(firstcap ? ((numname[numopt] || numname[0])).split("")[0].toUpperCase() + ((numname[numopt] || numname[0])).substring(1) : (numname[numopt] || numname[0]))}`;
};
